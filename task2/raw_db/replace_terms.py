...
def load_terms(path):
    """Читаем terms.json и готовим паттерны:
    1) фразы
...
def replace_terms(text, patterns):
...
def copy_with_replacements(src_root, dest_root, patterns):
    ...
    for dirpath, dirnames, filenames in os.walk(src_root):
        ...
        for name in filenames:
            ...
            # пробуем читать как текст; если не получается — копируем как есть
            try:
